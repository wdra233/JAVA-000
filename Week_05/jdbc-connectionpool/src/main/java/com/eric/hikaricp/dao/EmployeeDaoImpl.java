package com.eric.hikaricp.dao;

import com.eric.model.Employee;
import com.eric.preparestastatement.dao.EmployeeDao;
import com.eric.util.HikaricpUtils;
...
public class EmployeeDaoImpl implements EmployeeDao {
    ...
    public Employee getEmployeeById(int id) {
        ...
        try {
            connection = HikaricpUtils.getHikariCPConnection();
            ...
            HikaricpUtils.closeResource(connection, statement, resultSet);
        }
        return employee;
    }
    ...
    public boolean updateEmailById(String email, int id) {
        ...
        int resultCnts = 0;
        try {
            conn = HikaricpUtils.getHikariCPConnection();
            preparedStatement = conn.prepareStatement(sql);
            resultCnts = update(preparedStatement, email, id);
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
            HikaricpUtils.closeResource(conn, preparedStatement);
        }
    ...
    public boolean updateTransaction(String email, int id1, int id2) {
        ...
        try {
            ...
            // reset autoCommit as true before close
            HikaricpUtils.resetAutoCommit(conn);
            HikaricpUtils.closeResource(conn, preparedStatement);
        }
        return resultCnts > 0 ? true : false;
